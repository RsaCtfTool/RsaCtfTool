...
class Attack(AbstractAttack):
    ...
    def attack(self, publickeys, cipher=[], progress=True):
        ...
        for e in es:
            ...
            if pow(maybe_plaintext, e) == maybe_plaintext_to_the_e:  # Found the ciphertext
                plaintext = maybe_plaintext.to_bytes((maybe_plaintext.bit_length() + 7) // 8, "big")
    ...
    def test(self):
        ...
        result = self.attack(
            ...
            [cipher.to_bytes((cipher.bit_length() + 7) // 8, "big") for cipher in ciphers],
            progress=False
        )
