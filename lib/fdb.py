...
def send2fdb(composite, factors):
  ...
  headers = {'User-Agent': 'Mozilla/5.0', 'Connection': 'keep-alive', 'Content-Type': 'application/x-www-form-urlencoded'}
  response = http.request('POST', url, encode_multipart=False,headers=headers,fields=payload)
